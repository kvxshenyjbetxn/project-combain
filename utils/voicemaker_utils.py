...
def test_voicemaker_connection(app_instance):
    ...
    if balance is not None:
        ...
        vm_text = balance
        app_instance.root.after(0, lambda: app_instance.settings_vm_balance_label.config(text=f"{app_instance._t('balance_label')}: {vm_text}"))
        app_instance.root.after(0, lambda: app_instance.chain_vm_balance_label.config(text=f"{app_instance._t('voicemaker_balance_label')}: {vm_text}"))
        app_instance.root.after(0, lambda: app_instance.rewrite_vm_balance_label.config(text=f"{app_instance._t('voicemaker_balance_label')}: {vm_text}"))
